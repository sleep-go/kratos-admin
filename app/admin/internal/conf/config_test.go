...
func TestLoadReadsSetupFromYAML(t *testing.T) {
...
func TestNewConfigRejectsMissingProductionSecrets(t *testing.T) {
	if _, err := NewConfig(&Bootstrap{Environment: "production"}); err == nil {
		t.Fatal("NewConfig() error = nil, want missing secret error")
	}
}

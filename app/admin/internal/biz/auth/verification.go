...
func NewVerificationUsecase(repository VerificationRepository, users UserRepository, hasher *PasswordHasher, secret []byte, senders []MessageSender, now func() time.Time) (*VerificationUsecase, error) {
	if len(secret) < 16 {
	...
	registry := make(map[string]MessageSender, len(senders))
...
func (u *VerificationUsecase) issue(ctx context.Context, user User, target, scene, channel string, contextData json.RawMessage) (VerificationChallenge, error) {
	...
	if err := sender.SendCode(ctx, CodeMessage{Target: target, Scene: scene, Code: code, Minutes: 5}); err != nil {
		return VerificationChallenge{}, err
	}

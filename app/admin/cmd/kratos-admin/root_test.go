...
func TestWorkerCommandParsesConf(t *testing.T) {
	var got string
	cmd := newRootCommand(commandRunners{
		worker: func(_ context.Context, confPath string) error {
			got = confPath
			return nil
		},
	})
	cmd.SetArgs([]string{"worker", "--conf", "/etc/kratos-admin/worker.yaml"})
	if err := cmd.ExecuteContext(context.Background()); err != nil {
		t.Fatal(err)
	}
	if got != "/etc/kratos-admin/worker.yaml" {
		t.Fatalf("conf = %q", got)
	}
}

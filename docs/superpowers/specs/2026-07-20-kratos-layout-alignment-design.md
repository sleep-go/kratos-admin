...
```text
api/admin/v1/*.proto
app/admin/cmd/{server,initadmin,gormgen}
...
Admin 默认 `--conf ./configs/admin.yaml`，Worker 默认 `--conf ./configs/worker.yaml`，initadmin 默认复用 Admin 配置。Cobra 保留，并成为官方 `-conf` 语义的承载方式。
...
Docker 镜像复制 `configs/` 并以 `--conf /app/configs/admin.yaml` 或 `worker.yaml` 启动。Compose 继续通过 `.env` 提供敏感值和容器地址，YAML 不保存真实密钥。宿主机启动改为显式 `--conf`，原有 `KRATOS_ADMIN_*` 环境变量仍可用。
...
- Admin、Worker、initadmin 的 Cobra 测试覆盖 `--conf` 默认值和自定义路径。
- `make all`、`make build`、`make test`、前端测试和 Docker 镜像构建通过。
- `go run ./app/admin/cmd/server --conf ./configs/admin.yaml` 与 Worker 对应命令可启动。
